...
using static TruCompiler.CodeGeneration.CodeGenerationStackHelper;
using TruCompiler.Semantic_Analyzer.SymbolTableClasses;

namespace TruCompiler.CodeGeneration
{
    public class CodeGenerationStackVisitor : Visitor<Token>
    {
        ...
        {
            foreach (var child in node.Children){child.accept(this);}
            GeneratedCode[Component] += AddOp(node.Left[0], node.Right[0], node.Operation.Value.Value, node.Entry);
        }
        ...
        public override void visit(AssignStatementNode node)
        {
            ...
            if (node.Left.Children.Count >= 2)
            {
                var firstNode = node.Left[0];
                int leftOffset = firstNode.Entry.Offset;
                for (int i = 1; i < node.Left.Children.Count; i++)
                {
                    leftOffset += node.Left[i].Entry.Offset;
                }
                GeneratedCode[Component] += AssignSides(node.Left.Entry, right.Entry, leftOffset);
            }
            else
            {
        ...
        public override void visit(MainNode node)
        {
            ...
            GeneratedCode[Component] += String.Format("{0,-8} addi R14,R0,topaddr\n", "");

            foreach (var child in node.Children){child.accept(this);}
            GeneratedCode[Component] += "hlt\n%- PROGRAM END -%\n\n";
            GeneratedCode["Data"] += String.Format("{0,-8}buf res 20\n", "");
        }
        ...
        public override void visit(ReturnStatementNode node)
        {
            ...
            if (node.Expression.Children.Count > 0)
            {
                ...
                GeneratedCode[Component] += ReturnFrom(entry);
            }
        ...
        public override void visit(WriteStatementNode node)
        {
            ...
            GeneratedCode[Component] += WriteToConsole(writeNode);
        }
        ...
        public override void visit(RelExprNode node)
        {
            ...
            {
                left = node.LeftArithExpr[0];
            }
            if (node.RightArithExpr.Children.Count > 0)
            {
                right = node.RightArithExpr[0];
            }
        ...
        public override void visit(ReadStatementNode node)
        {
            ...
            while (readNode.Entry == null && readNode.Value.Value != "Variable")
            {
            ...
            GeneratedCode[Component] += ReadFromConsole(readNode);
        }
    }
}
